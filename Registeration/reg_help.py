# Code for "An Application of Fourier-Mellin Transform in Image Registration" paper written by Xiaoxin Guo, Zhiwen Xu, Yinan Lu, Yunjie Pang
# This code was made possible thanks to https://github.com/polakluk/fourier-mellin 
...
COLS_AXIS = 1
polarMode = "spline"
noiseMode = "none" # "none" "gaussian", "s&p", "none"
noiseIntensity = {'sigma' : 2.5, 'mean' : 0, 'whiteThreshold' : 0.01, 'blackThreshold' : 0.99}
...
def calc_moments(img): # make sure input image is binary
    ...
    if len(contours)>0:
        Humoments = cv2.HuMoments(cv2.moments(contours[0]))
    else:
        Humoments = [100000] # just some high value
    
    return Humoments[0]

def register_main(source_img, target_img, infer = 0):   
    # Processing Calls start here:
    
    # Step 0: Pad to Same Shape
    source_img, target_img = pad_to_same_shape(source_img, target_img)
    # Step 1 - Apply FFT on both images and get their magnitude spectrums
    # image (we are looking for), lets call it original
    imgOriginal, imgOriginalFft, imgOriginalMags = processImage(target_img)

    # image (we are searching in), lets call it transformed
    imgTransformed, imgTransformedFft, imgTransformedMags = processImage(source_img)
    ...
    if infer == 1:
        mdic = {"Angle_Deg": angleDeg, "Scale": scaleFactor, "X":-x, "Y":-y}
    else:
        match_score = abs(calc_moments(imgFinal) - calc_moments(target_img)) # works only for binary images
        mdic = {"Similarity_Score":match_score, "Angle_Deg": angleDeg, "Scale": scaleFactor, "X":-x, "Y":-y}
    
    return imgFinal, mdic
